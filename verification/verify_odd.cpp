...
// in range admits NUM_WITNESSES square-free summands ell with n - ell prime and with
...
//   are, by construction, NUM_WITNESSES pairwise-disjoint witnesses, so no
...
//   collects NUM_WITNESSES pairwise-disjoint witnesses. This tier uses the
...
const i64 RANGE_START = 10001LL;         // 108 <= n <= 10^4 is settled EXACTLY by
                                         // Threshold.py (the covering criterion), not by
                                         // the witness device: the two differ, and at
                                         // n = 167 the device fails although n = 167 is
                                         // good.  Nothing below can be inherited from
                                         // Lee--O'Clarey either, whose small-range lemmas
                                         // supply at most four witnesses.
...
// NUM_WITNESSES pairwise-coprime square-free witnesses dodge every k with
// omega(k) <= NUM_WITNESSES - 1.  Six is what Theorem 1.3 (omega(k) <= 5)
// requires.  This is the ONLY place the count appears -- change it here and
// nowhere else, and keep verify_results.py's copy in step with it.
constexpr int NUM_WITNESSES = 6;
...
// a very large safety margin for finding NUM_WITNESSES hits at n up to 2e12.
...
// ============================================================================
// Fallback: exhaustive NUM_WITNESSES-witness search
// ============================================================================
...
bool check_witnesses(i64 w) {
    vector<OddSupport> reps;
    reps.reserve(NUM_WITNESSES);
    ...
    for (i64 pr : FALLBACK_PRIMES) {
        ...
        if (ok) {
            reps.push_back(sup);
            if ((int)reps.size() == NUM_WITNESSES) return true;
        }
    }
    return false;
}
...
bool fast_witnesses(i64 w) {
    ...
    for (i64 ell : WITNESS_POOL) {
        ...
        if (is_prime((u64)(w - ell))) {
            count++;
            if (count >= NUM_WITNESSES) return true;
        }
    }
    return false;
}
...
int main(int argc, char* argv[]) {
    ...
    cout << "============================================================" << endl;
    cout << "Odd n: " << NUM_WITNESSES << " pairwise coprime square-free witnesses" << endl;
    cout << "============================================================" << endl;
    ...
    if (test_mode) {
        cout << "\n=== TEST MODE ===" << endl;
        cout << "Testing check_witnesses(100001)..." << endl;
        cout << "  Result: " << (check_witnesses(100001) ? "True" : "False") << endl;
