...
// NUM_SPLITS distinct Goldbach splits supply 2*NUM_SPLITS distinct primes,
// hence that many pairwise-coprime witnesses, dodging every k with
// omega(k) <= 2*NUM_SPLITS - 1.  Three splits give six primes and so cover
// omega(k) <= 5.  Note the ladder steps only at even omega: two splits cover
// omega(k) <= 3, three cover omega(k) <= 4 AND omega(k) <= 5.  This is the ONLY
// place the count appears -- keep verify_results.py's copy in step with it.
const int NUM_SPLITS  = 3;

const i64 RANGE_START = 10002LL;           // 108 <= n <= 10^4 is settled EXACTLY by
                                           // Threshold.py; see verify_odd.cpp.  Nothing
                                           // below can be inherited from Lee--O'Clarey
                                           // either (their Lem. 2.4 supplies two splits,
                                           // i.e. omega(k) <= 3 only).
...
// ============================================================================
// Exhaustive NUM_SPLITS-split check
//
// Returns up to NUM_SPLITS distinct primes q with n - q also prime (the split
// witnesses q and its partner n-q); a result of that size certifies n. Used
...
vector<i64> distinct_splits(i64 n) {
    vector<i64> found;
    for (i64 q = 2; 2 * q <= n; q++) {
        if (is_prime((u64)q) && is_prime((u64)(n - q))) {
            found.push_back(q);
            if ((int)found.size() == NUM_SPLITS) return found;
        }
    }
    return found;
}
        ...
        if (sieve_reps < NUM_SPLITS) {
            ...
            for (i64 q : FALLBACK_PRIMES) {
                ...
                if (pc_prime_set.count(m - q)) {
                    fallback_reps++;
                    if (fallback_reps >= NUM_SPLITS) break;
                }
            }

            if (fallback_reps < NUM_SPLITS) {
                // Tier 3 exhaustive fallback: rigorous NUM_SPLITS-split search.
                vector<i64> splits = distinct_splits(m);
                if ((int)splits.size() < NUM_SPLITS) {
...
void run_self_tests() {
    ...
    // 3. distinct_splits returns NUM_SPLITS genuine, pairwise-distinct splits
    //    when they exist, and each reported q gives a prime partner n-q.
    {
        i64 n = 4810000000LL;
        auto s = distinct_splits(n);
        bool ok = (int)s.size() == NUM_SPLITS;
        for (size_t i = 0; ok && i < s.size(); i++)
            for (size_t j = i + 1; ok && j < s.size(); j++)
                ok = s[i] != s[j];
        for (i64 q : s) ok = ok && is_prime((u64)q) && is_prime((u64)(n - q));
        cout << "  [3] distinct_splits gives NUM_SPLITS valid distinct splits: "
             << (ok ? "PASS" : "FAIL") << endl;
        if (!ok) { cerr << "REGRESSION FAILURE: test 3\n"; exit(1); }
    }

    // 4. The value the literature flags as sporadic, n = 7,740,000,088, is
    //    cleared by the split search (it has many distinct-prime splits; it
    //    only looks hard under a small-prime-only search).
    {
        i64 n = 7740000088LL;
        auto s = distinct_splits(n);
        bool ok = (int)s.size() == NUM_SPLITS;
        cout << "  [4] n=7740000088 cleared by the split search: "
             << (ok ? "PASS" : "FAIL") << endl;
...
int main(int argc, char* argv[]) {
    ...
    cout << "============================================================" << endl;
    cout << "Even n: " << NUM_SPLITS << " distinct Goldbach splits" << endl;
    cout << "============================================================" << endl;
    ...
    if (test_mode) {
        ...
        cout << "distinct_splits(740000138): ";
        auto s1 = distinct_splits(740000138);
        for (i64 q : s1) cout << q << "(" << (740000138 - q) << ") ";
        cout << (s1.size() >= 2 ? " -> OK" : " -> INSUFFICIENT") << endl;

        cout << "distinct_splits(7740000088): ";
        auto s2 = distinct_splits(7740000088LL);
